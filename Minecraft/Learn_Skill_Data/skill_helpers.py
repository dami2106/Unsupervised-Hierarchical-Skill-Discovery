...
def make_skill_segments(acts, truth):
    """
    Build segments with 0-based inclusive indices.
    A segment ends either when:
      1) the action is in `segment_actions`, or
      2) the truth label changes, or
      3) we reach the last frame (n-1).
    """
    n = len(acts)
    if len(truth) != n:
        raise ValueError(f"acts/truth length mismatch: acts={n}, truth={len(truth)}")

    segment_actions = set(range(5, 17))  # boundary actions

    # 0-based inclusive ends from actions
    action_ends = [i for i, a in enumerate(acts) if a in segment_actions]

    # 0-based inclusive ends from truth transitions:
    # if truth changes at i, prior segment ends at i-1
    truth_change_ends = [i - 1 for i in range(1, n) if truth[i] != truth[i - 1]]

    # Always close the final segment at n-1
    ends = sorted(set(action_ends) | set(truth_change_ends) | {n - 1})

    # Build segments [prev_end+1 .. end], all indices in [0, n-1]
    bounds = [-1] + ends
    idx_groups = [
        list(range(lo + 1, hi + 1)) for lo, hi in zip(bounds[:-1], bounds[1:])
    ]

    # Group by label (now guaranteed uniform within each segment)
    skill_segments = {}
    for seg in idx_groups:
        label = truth[seg[0]]
        # Sanity check (can keep or remove once confident)
        if any(truth[i] != label for i in seg):
            raise ValueError(
                f"Unexpected mixed labels after splitting in segment {seg}"
            )
...
def evaluate_ocsvm(X_pos_train, X_pos_val, X_neg_val, nu, gamma):
    ...
    y_true = np.hstack(
        [np.ones(len(X_pos_val), dtype=int), np.zeros(len(X_neg_val), dtype=int)]
    )
...
def load_models(models_dir):
    ...
    for fname in os.listdir(models_dir):
        ...
        models[skill] = {"model": model, "threshold": threshold, "meta": meta}
    return models


def classify_state_with_start_models(
    state_vec, start_models, strategy="prob", return_all=False
):
...
def build_startability_dataset(
    dir_: str,
    skill: str,
    files,
    features_dirname="pca_features_512",
    old_data_mode=False,
    skills_dir="groundTruth",
):
    """
    ...
    for file in files:
        ...
        # --- keep labels aligned to features ---
        if len(acts) > n_frames:
            acts = acts[:n_frames]
            truths = truths[:n_frames]
        elif len(acts) < n_frames:
            feats = feats[: len(acts)]  # rare, but keep alignment if feats are longer
            n_frames = feats.shape[0]

        # print("Length of acts after shift:", len(acts))
        # print("Shape of acts after shift:", acts.shape)
        # print("Length of truths after shift:", len(truths))

        segs_for_skill = make_skill_segments(acts, truths)

        # print("Segments for skill:", segs_for_skill)
    ...
    g_neg = (
        np.concatenate(neg_groups, axis=0)
        if len(neg_groups)
    ...
    for file in files:
        ...
        if skill in segs_for_skill:
            for seg in segs_for_skill[skill]:
                ...
                X_pos.append(feats[seg[-1]][None, :])
                groups_pos.append(file)
                # negative within-skill = all but last
                if len(seg) > 1:
                    X_neg.append(feats[seg[:-1]])
                    groups_neg.extend([file] * (len(seg) - 1))
        ...
        for other_skill, segs in segs_for_skill.items():
            if other_skill == skill:
                continue
            for seg in segs:
                X_neg.append(feats[seg])
                groups_neg.extend([file] * len(seg))
...
def applicable_pu_start_models(models, state, *, return_details=False, eps=0.0):
    ...
    if state.ndim == 1:
        ...
        raise ValueError(
            "`state` must be a single feature vector of shape [d] or [1,d]."
        )
    ...
    if applicable:
        ...
        for r in applicable:
            print(
                f"  - {r['skill']}: p={r['prob']:.3f}  thr={r['thr']:.3f}  margin={r['margin']:.3f}"
            )
    else:
...
def end_state_prob_pu(models_dir: str, skill: str, state) -> dict:
    ...
    margin = p - thr
    return {"prob": p, "threshold": thr, "is_end": bool(p >= thr), "margin": margin}
